<template>
  <div>
    <v-card-title class="headline">New Trends</v-card-title>
      ...
      <v-row>
        <v-col v-for="book in books" :key="book.id" cols="12" md="4">
          <v-card class="pa-4" elevation="2">
            ...
            <v-card-text>
              <v-btn @click="openTrend(book)" color="primary"> View </v-btn>
...
export default {
  ...
  methods: {
    async getnewtrends() {
        try {
          const response = await axios.get(`${process.env.apiUrl}books/get_trends`);
    ...
    async additem(selectedBook) {
      try {
        const response = await axios.post(`${process.env.apiUrl}books/add_to_trends/${selectedBook.accessionNo}`);
                if (response.status === 200) {
          const bookDetails = response.data;
          console.log('Book details:', bookDetails);
          this.dialog = false;
          alert("Book added to top trends");
    ...
    async remove(selectedBook) {
      try {
        const response = await axios.post(`${process.env.apiUrl}books/remove_from_trends/${selectedBook.accessionNo}`);
                if (response.status === 200) {
          const bookDetails = response.data;
          console.log('Book details:', bookDetails);
          this.dialog = false;
          alert("Book removed from top trends");
        } else {
          console.error('Failed to remove book details. Unexpected status code:', response.status);
        }
      } catch (error) {
        console.error('Error removing book details:', error);
      }
    },
    async searchData() {
      try {
